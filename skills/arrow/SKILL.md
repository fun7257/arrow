...
description: >
  ...
  target response writers (developer-defined bodies), ServeMux routing. Read this
  skill when working in the arrow repo, adding routes/middleware/handlers,
  using the target package, mounting net/http handlers via HandleHTTP, or when
  the user mentions Arrow, 穿透, or
...
| `target.Abort` / `AbortJSON` | Writes body then Abort — use in **middleware/auth** |
| `target.WriteJSON` etc. | Handler response **without** Abort |
...
| Handler error (no stop) | `target.WriteJSON(c, status, yourBody)` |
| Middleware auth fail | `target.AbortJSON(c, status, yourBody)` + `return` |
...
**DO**: `target` for JSON APIs · group-level auth · `HandleHTTP` for stdlib handlers · write middleware as `HandlerFunc` + `c.After`

**DON'T**: onion `func(http.Handler) http.Handler` middleware · onion After/LIFO assumptions · `Mux().HandleFunc` for app routes · `middleware.Auth()` · double-write after `target.Write*` · third-party routers
...
```
arrow.go router.go context.go pipeline.go group.go
